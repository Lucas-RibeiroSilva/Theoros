...
export default function CardFull({ handleLogout }) {
    ...
    try {
      ...
      if (error.response) {
        showAlertWithTimeout(
          `Erro do servidor: ${error.response.data?.error || error.message}`,
        );
  ...
  return (
    <>
      ...
      {showAlertFavorite && (
        <div
          className={`popup-alert-favorite-overlay ${isClosingAlert ? "closing" : ""}`}
          onClick={closeAlert}
        >
          <div
            className={`alert-favorite-dialog ${isClosingAlert ? "closing" : ""}`}
